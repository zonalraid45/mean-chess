...
(function () {
  ...
  const CONFIG = {
    ...
    repo: 'YOUR_REPO_NAME',
    eventType: 'analyze-position',
    // Put the token VALUE here (not the secret name). If your repo secret is GH,
    // copy that same token value into this field for browser-side dispatch.
  ...
  function validateConfig() {
    ...
    if (CONFIG.githubToken === 'GH') {
      console.warn('[MeanChess] `GH` is a secret name, not a token value. Browser scripts cannot read GitHub repo secrets.');
      missing.push('githubToken(value)');
    }
    if (missing.length > 0) {
      console.warn('[MeanChess] Missing config fields:', missing.join(', '));
    }
    return missing;
  }
